...
@RequestMapping("/post") // Mapping: Controller의 메서드에서만 해왔었음. 클래스의 매핑의 경우: PostController 클래스의 메서드들은 요청
							// 주소가 "/post"로 시작.
@Controller // DispatcherServlet에게 Controller를 컴포넌트로 등록. <- package com.itwill.spring2.web
			// 패키지 스캔, @Controller -> DispatcherServlet이 사용 가능하게 됨.
public class PostController {

	private final PostService postService; // 생성자에 의한 의존성 주입. Controller는 Service 사용. Controller는 Service의 메서드를 사용하기 위해
											// Service를 변수로 가지고 있어야 함.
	...
	public void list(Model model) {
		...
		model.addAttribute("posts", list); // `posts`: JSP에서 el로 사용 가능.

		// 리턴 값이 없는 경우 뷰의 이름은 요청 주소와 같음
		// 경로: WEB-INF/views/post/list.jsp, servlet-context.xml에서 접두사, 접미사 설정.
	}
