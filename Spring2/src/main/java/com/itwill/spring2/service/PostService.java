...
// service에서 repository(dao)를 사용함
// application-context.xml 파일에서 <context:component-scan> 설정에서 
...
//  @Autowired private PostRepository postRepository;  1. 필드에 의한 의존성 주입
    ...
    // service에서 repository(dao)를 사용함. 미리 만들어져 있을 거라 생각 -> 만들어진 객체에서 메서드 호출 
...
//        return list.stream().map(PostListDto::fromEntity).toList(); 
        ...
        // PostListDto 객체에 있는 메서드(좌우 변경 가능)
        ...
        // 익명 내부 클래스: 람다 표현식 + list가 하나씩 PostListDto로 넘어감. 반복문.
    }
