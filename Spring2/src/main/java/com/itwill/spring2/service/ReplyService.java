...
@Slf4j
@RequiredArgsConstructor // final field 초기화.
@Service // Spring Context에 서비스 컴포넌트 객체로 등록.
public class ReplyService {
	private final ReplyRepository replyRepository; // Spring Context가 @을 보고 생성 및 초기화 > 생성자에 의한 의존성 주입.
	...
	public List<ReplyReadDto> read(long postId) {
		...
		// ReplyReadDto::fromEntity == (x) -> ReplyReadDto.fromEntity(x)

		// 다음 표기법을 사용할려면 지켜야 하는 규칙.
		// 1. 변경 없이 argument를 전달할 경우, argument가 1개만 있을 경우 사용.
		// 2-1. ReplyReadDto 객체에서 메서드 호출이거나, (예) String::length -> (x) -> x.length()
	...
	public int update(long id, ReplyUpdateDto dto) {
		...
		Reply entity = Reply.builder().id(id).reply_text(dto.getReplyText()).build();
