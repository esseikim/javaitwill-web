...
<html>
    ...
    <body>
        ...
        <main>
            ...
            따로 지정하지 않으면, 현재 보고 있는 페이지 그대로를 요청을 다시 보내는 것. -->
                <div> <!-- 요청파라미터에 보낼 name-->  
                    <input id="id" name="id" 
                        type="text" value="${ post.id }" readonly /><!-- PostModifyController에서 setAttribute한 이름 이용-->         
                </div>
                <div>
                ...
                <%-- 버튼이 사라져서 클릭 불가능. post 방식에서 update delete 불가능. 권한을 가진 사용자들만 들어올 수 있도록. --%>
                        <div>
                            ...
                            <!-- input type submit이랑 동일함. form 안의 버튼: form을 서브밋함(fromr의 기본동작 적용). form 밖에 있으면 submit이 안됨. click만 될 뿐
                            submit은 기본임! form 안에 있는 button은 ! -->
                           <!-- 이벤트 핸들러를 js를 이용해 form 안에 있든 바깥에 있든 등록해야함. 하는 일이 다르기 때문에. -->
        ...
        <script src='../js/post-modify.js'></script> <!-- 한단계 올라가서 js 폴더 찾고, 파일 이름 찾아들어감  -->
    </body>
<!-- 가능하면 js는 html body안에 작성하지 않고 별도의 파일로 만드는 걸 추천함(긴 경우) 
jsp는 컨트롤러에서 포워딩해서 올것: 웹inf에 작성가능(서버 내부에서만 접속 가능한 폴더), 외부 불가
