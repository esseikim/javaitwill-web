...
public class AuthenticationFilter extends HttpFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) // FilterChain이라는 객체를 전달받음
           ...
           // (2) 세션에 로그인 정보(signedIn;User)가 있는 지를 확인. <- signedInUser(변수이름)- null이 아닌 경우: 로그인. 
          String username = (String) session.getAttribute("signedInUser") ; 
          // 세션에 저장할 떄 해당 변수로 저장했으니 가져올 때도 동일한 변수로.
         ...
         // 로그인 정보가 없으면, 로그인 페이지로 redirect (contextroot 같이 써줘야.). 필터 클래스 생성 -> 서버 재시작 
